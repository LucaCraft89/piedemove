...
Future<List<File>> fetchOsmTiles(
  ...
  try {
    for (final tile in tiles) {
      ...
      final body = await _fetchTile(c, tile, log);
      if (body == null) {
...
Future<String?> _fetchTile(http.Client c, OsmTile tile, OsmLog? log) async {
  ...
  for (var attempt = 0; attempt < order.length; attempt++) {
    ...
    try {
      ...
      if (r.statusCode == 200 && r.bodyBytes.length > 2048) {
        log?.call('${tile.name} ${(r.bodyBytes.length / 1e6).toStringAsFixed(1)} MB');
        return r.body;
      }
      log?.call('${tile.name} HTTP ${r.statusCode} from $endpoint');
      if (r.statusCode == 429) {
        // Overpass is rate limiting: back off well beyond the usual retry.
        await Future<void>.delayed(const Duration(seconds: 60));
      }
    } catch (e) {
      log?.call('${tile.name} $endpoint failed: $e');
    }
